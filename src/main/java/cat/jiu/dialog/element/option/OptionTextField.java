...
import cat.jiu.dialog.element.option.draw.GuiOptionTextField;
 ...
 */
@ZenRegister
@ZenClass("dialog.option.Text")
public class OptionTextField implements IDialogOption {
	 ...
	 */
	public OptionTextField(IText info, boolean closeDialog) {
		this.text = info!=null ? info : Text.empty;
	 ...
	 */
	public OptionTextField(IText text, boolean closeDialog, IText... tooltips) {
	 ...
	 * @param tooltips 鼠标移动到文本框上时显示的提示
	 */
	public OptionTextField(IText text, boolean closeDialog, List<IText> tooltips) {
		this.text = text!=null ? text : Text.empty;
		this.closeDialog = closeDialog;
	...
	public NBTTagCompound writeToNBT() {
		...
		if(this.text!=null) nbt.setTag("text", this.text.writeTo(NBTTagCompound.class));
	...
	public ResourceLocation getTypeID() {
		return DialogOption.text().getTypeID();
	}
	@Override
	public OptionTextField copy() {
		List<IText> tooltips = this.tooltips == null ? null : this.tooltips.stream().map(tooltip -> tooltip.copy()).collect(Collectors.toList());
		return new OptionTextField(this.text.copy(), canCloseDialog(), tooltips);
	}
	@SideOnly(Side.CLIENT)
	@Override
	public OptionDrawUnit getDrawUnit(ResourceLocation dialogID, int optionID, IDialogOption option, DialogDimension dialogDimension) {
		return new GuiOptionTextField(Minecraft.getMinecraft().player, dialogID, (OptionTextField) option, optionID, dialogDimension);
	}
}
